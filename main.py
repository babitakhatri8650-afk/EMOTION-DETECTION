...
"""
2. Preprocess the upcoming text clean raw text so it matches the format used while training.
A. Convert the text to lowercase. -done
B. Remove apostrophes . -done
...
async def lifespan(app: FastAPI):
    ...
    dl_model.clear() #  ek bar server band hone k bad remove the model from memory

"""
5. Mount the Static Files to the FastAPI app
a. enable CORS(cross-Origin Resource Sharing) to all requests from different origins."""
...
def predict_emotion(text_input:TextInput):
    """
    a. Cleans the input sentences.
    b. Convert the words into numeric using tokenizer.
    c. Pad the sequences to ensure uniform length.
    d. Run prediction using the BiGRU model.
    e. Return the top emotion and full probability breakdown."""
    ...
    top_emotion_index = int(np.argmax(probabilities)) #4
    all_probabilities ={
